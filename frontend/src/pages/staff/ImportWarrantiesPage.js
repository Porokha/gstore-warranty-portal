...
const ImportWarrantiesPage = () => {
  ...
  const uploadMutation = useMutation(
    ...
    },
    {
      ...
      onError: (err) => {
        setError(err.response?.data?.message || 'Failed to import warranties');
      },
    }
  );
    ...
    <Container maxWidth="md">
      <Box sx={{ mt: 4, mb: 4 }}>
        <Box display="flex" alignItems="center" gap={2} mb={3}>
          ...
          <Typography variant="h4">Import Warranties from CSV</Typography>
        </Box>

        <Paper sx={{ p: 3 }}>
          <Box mb={3}>
            <Typography variant="body1" gutterBottom>
              Upload a CSV file to import multiple warranty products at once.
            </Typography>
            <Button
              variant="outlined"
          ...
          <Box mt={3} display="flex" gap={2}>
            ...
            >
              {uploadMutation.isLoading ? 'Importing...' : 'Import Warranties'}
            </Button>
...
export default ImportWarrantiesPage;
